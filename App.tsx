...
export default function App() {
  ...
  const handleLogout = () => {
    ...
    toast.success(
      "Logout realizado com sucesso! Você pode continuar navegando no feed público.",
    );
  };
  ...
  const handleLoginRedirect = () => {
    ...
    navigateTo("auth");
    toast.info(
      "Faça login para solicitar jogos e acessar todas as funcionalidades",
    );
  };
  ...
  const handleGameRequest = (gameId: string) => {
    if (!state.isLoggedIn) {
      toast.error(
        "Você precisa estar logado para solicitar jogos",
      );
      setState((prev) => ({ ...prev, authMode: "login" }));
      navigateTo("auth");
      return;
  ...
  const handleRequestSent = (requestData: any) => {
    ...
    // Toast de feedback
    toast.success(
      "Solicitação enviada com sucesso! O time será notificado.",
    );
  };
  ...
  const handleRequestGameWithDate = (
    teamId: string,
    date: Date,
  ) => {
    if (!state.isLoggedIn) {
    ...
    toast.success(
      `Solicitação enviada para ${date.toLocaleDateString("pt-BR")}!`,
    );
  };
  ...
  const handleAddAvailableDate = (date: Date) => {
    toast.success(
      `Data ${date.toLocaleDateString("pt-BR")} marcada como disponível!`,
    );
  };

  const handleRemoveAvailableDate = (date: Date) => {
    toast.success(
      `Data ${date.toLocaleDateString("pt-BR")} removida da disponibilidade!`,
    );
  };
        ...
        return (
          <TeamProfile
            ...
            onBack={() => {
              ...
              const returnTo =
                (state as any).returnTo || "search-results";
              navigateTo(returnTo);
            }}
